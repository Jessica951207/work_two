...
function addMobtechVisitors() {
    $.ajax({
        ...
        data: {"ip":returnCitySN["cip"], "cityId":returnCitySN["cid"], "cityName":returnCitySN["cname"], "type":"10"},
...
$(".button").click(function () {
    ...
    var cityArray=InputArray[1].split(" ");
    ...
    for (var val in InputArray){
            ...
            // 手机号验证
             if(!checkMobile(InputArray[val]) && (val == 4)){
                $(Input_label[val]).siblings('.error-message').show();
            ...
            //推荐码验证
        if(!checkReferral(InputArray[val]) && (val == 5)){
            $(Input_label[val]).siblings('.error-message').show();
            $(Input_label[val]).siblings('.error-message').html($(Input_label[val]).data('invalid'));
            setTimeout(function () {
                $(Input_label[val]).siblings('.error-message').fadeOut();
            ...
            if(checkReferral(InputArray[val]) && (val == 5)) {
                ...
                submit = true;
                 $.ajax({
                     ...
                     url: './mock.json',
                     type:"get",//todo later 更改为post
                     data: JSON.stringify({'mainUser':InputArray[0], 'businessProvince':province, 'businessCity':homeCity, 'remark1':InputArray[2], 'mainLiveProvince':province, 'mainLiveCity':homeCity, 'mainPhone':InputArray[4], 'remark2': InputArray[5], 'loanTerm':36, 'loanAmount':1, 'businessType':3, 'businessChannelCooperation':10, 'businessApplyMode':1}),
                     success:function (data) {
                         console.log("提交成功！",province,homeCity)
                         if (data.resCode == 200 && data.data == 1) {
                             window.location.href = '../web/gzCarWelcome_success.html';
